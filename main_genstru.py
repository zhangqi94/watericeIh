...
# ----------------------------------------------------------------------------------------------
# Optional "kick" to enhance exploration
# ----------------------------------------------------------------------------------------------
parser.add_argument(
    "--kick_interval_blocks",
    type=int,
    default=0,
    help="Apply a random displacement kick every N blocks (0 disables).",
)
parser.add_argument(
    "--kick_sigma",
    type=float,
    default=0.05,
    help="Kick displacement standard deviation in Å (applied to all atoms).",
)
...
kick_interval_blocks: int = args.kick_interval_blocks
kick_sigma: float = args.kick_sigma
...
print(f"{'Kick interval (blocks):':22s} {kick_interval_blocks}")
print(f"{'Kick sigma (Å):':22s} {kick_sigma:.3f}")
...
from source.updateblock import run_mc_continuous_block, run_mc_only_loop_block
from source.tools import safe_div, fmt_rate, calculate_pressure_from_stress, wrap_pos
...
for block in range(1, num_blocks + 1):
    ...
    # ----------------------------- LOOP stage (Force Flip) -----------------------------
    # Performs force flip hydrogen reorientations without Metropolis testing.
    print("\n" + "=" * 80, flush=True)
    for flip_step in range(num_loop_steps):
        state_curr, atoms = perform_loop_flip(
            state_curr,
            atoms,
        )
        hex_state = bitstring_to_hexstr(state_to_bitstring(state_curr))
        if print_interval_loop > 0 and (flip_step + 1) % print_interval_loop == 0:
            print(f"[FORCE FLIP]  step={flip_step+1:6d}/{num_loop_steps}  state={hex_state}", flush=True)

    # Optional random kick to enhance exploration (applied to all atoms)
    if kick_interval_blocks > 0 and (block % kick_interval_blocks == 0):
        coords_kick = atoms.get_positions()
        coords_kick = coords_kick + np.random.normal(scale=kick_sigma, size=coords_kick.shape)
        coords_kick = wrap_pos(coords_kick, atoms.cell.lengths())
        atoms.set_positions(coords_kick)
        print(f"[KICK] Applied random kick at block {block} (sigma={kick_sigma:.3f} Å)", flush=True)
    ...
    print(
        ...
        f"  Force flips:     {num_loop_steps} per block (no Metropolis)\n"
    ...
    append_block_summary_line(
        ...
        loop_accepts_block=0,  # No longer tracking loop accepts (force flip)
        loop_attempts_block=0,  # No longer tracking loop attempts (force flip)
